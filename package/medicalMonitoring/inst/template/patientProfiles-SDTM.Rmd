...
```{r runDocument-example, eval = FALSE}
...
```{r formatData-DM, eval = ("DM" %in% names(dataAll))}
	...
	# input parameters
	varsDM <- c("SEX", "RACE", "COUNTRY", "ETHNIC", "SITEID", "AGE")
...
```{r createPatientProfiles-dm, eval = "dm" %in% patientProfileModules}
...
```{r formatData-MH, eval = ("MH" %in% names(dataAll))}
	...
	if(length(setdiff(unique(dataMH$MHENRF), c("BEFORE", "DURING/AFTER", ""))) > 0)
		stop("Term in MHENRF are not among standard terms.")
	dataMH$MHENDTC <- trimws(with(dataMH,
		paste0(
			MHENDTC,
			if("MHENRF" %in% colnames(dataMH))
				ifelse(MHENRF != "", paste0(" (", tolower(MHENRF), ")"), "")
		)
	))
	labelVars[c("MHSTDTC", "MHENDTC")] <- c(
		"Start", 
		paste0("End", if("MHENRF" %in% colnames(dataMH))
			paste0("\n(", labelVars["MHENRF"], ")")
		)
	)
	
	idxMissingMHTERM <- which(dataMH[["MHTERM"]] == "")
	if(length(idxMissingMHTERM) > 0){
		message(paste("Missing MHTERM for", length(idxMissingMHTERM), "records, replaced by: 'Unknown'."))
		dataMH[idxMissingMHTERM, "MHTERM"] <- "Unknown"
	}
	...
	if(!is.null(dataMH) && length(paramValueVarMH) > 0 && length(paramGroupVarMH) > 0){
...
```{r createPatientProfiles-mh, eval = "mh" %in% patientProfileModules}
...
```{r formatData-CM, eval = ("CM" %in% names(dataAll))}	
...
```{r createPatientProfiles-cm, eval = "cm" %in% patientProfileModules}
...
```{r formatData-EX, eval = ("EX" %in% names(dataAll))}	
...
#	timeStartShapeVarEX <- "EXSTST"
#	timeEndShapeVarEX <- "EXENST"
...
```{r createPatientProfiles-ex, eval = "ex" %in% patientProfileModules}
...
```{r formatData-AE, eval = ("AE" %in% names(dataAll))}	
...
```{r createPatientProfiles-ae, eval = "ae" %in% patientProfileModules}
...
```{r formatData-LB, eval = ("LB" %in% names(dataAll))}	
...
```{r createPatientProfiles-lb, eval = "lb" %in% patientProfileModules}
...
```{r createSubjectProfileReport, message = TRUE, warning = TRUE}
