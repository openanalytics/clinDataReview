...
```{r utilityFcts}
	...
	formatDateFlagVariables <- function(x, type = c("start", "end")){
		type <- match.arg(type)
...
```{r patientProfiles-formatData-MH, eval = (c("ADMH", "MH") %in% names(dataAll))}
	...
	varMH <- if(varMHCode %in% colnames(dataMH)){
		idxEmptyCode <- which(dataMH[[varMHCode]] == "")
		if(length(idxEmptyCode) & "MHTERM" %in% colnames(dataMH)){
...
```{r patientProfiles-formatData-CM, eval = any(c("ADCM", "CM") %in% names(dataAll))}	
	...
	if("ASTFLG" %in% colnames(dataCM)){
		...
		flRes <- formatDateFlagVariables(x = dataCM[[timeStartShapeVarCM]], type = "start")
	...
	if("AENFLG" %in% colnames(dataCM)){
		...
		flRes <- formatDateFlagVariables(x = dataCM[[timeEndShapeVarCM]], type = "end")
