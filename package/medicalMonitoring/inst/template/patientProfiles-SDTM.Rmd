...
```{r utilityFcts}
	...
	# color/shape palette for laboratory abnormality
	shapePaletteLabInd <- c(
...
```{r patientProfiles-formatData-MH, eval = (c("ADMH", "MH") %in% names(dataAll))}
...
```{r patientProfiles-formatData-EX, eval = (c("ADEX", "EX") %in% names(dataAll))}	
...
```{r patientProfiles-formatData-LB, eval = any(c("ADLB", "LB") %in% names(dataAll))}	
	...
	if("PARAM" %in% colnames(dataLB)){
		...
		varsLB <- c("LBTEST", "LBSPEC", "LBMETHOD", "LBSTRESU")
	...
	paramVarLB <- varsLB
	timeVarLB <- intersect(c("ADY", "LBDY"), colnames(dataLB))
	...
	if(length(colorVarLB) > 0){
		...
		dataLB[[colorVarLB]] <- convertAesVar(data = dataLB, var = colorVarLB)
		levels <- levels(dataLB[[colorVarLB]])
		levels[is.na(levels)] <- 'NA'
		# add extra elements if present
		xStandard <- intersect(names(colorPaletteLabInd), levels)
		xExtra <- setdiff(na.omit(levels), names(colorPaletteLabInd))
		if(length(xExtra) > 0)
			warning("Flag variable contain non-standard elements:", toString(xExtra), ".")
		levels <- c(xStandard, xExtra)
		shapePaletteLB <- c(shapePaletteLabInd[xStandard], setNames(seq_along(xExtra), xExtra))
		colorPaletteLB <- c(colorPaletteLabInd[xStandard], setNames(seq_along(xExtra), xExtra))
		dataLB[[colorVarLB]] <- factor(dataLB[[colorVarLB]], levels = na.omit(levels), exclude = TRUE)
		
	}else	colorVarLB <- shapeVarLB <- colorPaletteLB <- shapePaletteLB <- NULL
		
	hasLBData <- !is.null(dataLB) && length(paramVarLB) > 0 && length(timeVarLB) > 0
	if(hasLBData){
