...
```{r countsVisualizationTemplate-computeSummaryStatisticsTable}
			...
			if(!is.null(names(colorVar))){
				dataTotalRow <- sapply(varWithTotal, function(varI){
					...
					dataTotalRow <- ddply(dataI, c("USUBJID", parentVar), function(x){
						...
						x[[colorVar[countVar]]] <- if(is.na(colorParentVar)){
							NA
	...
	labelVars[c("%", "statPercN")] <- paste(c(
		...
		if(!is.null(loopingVar) && loopingTotal)	
			paste("by", toString(getLabelVar(loopingVar, labelVars = labelVars)))
	), collapse = " ")
	labelVars[c("m", "statm")] <- "Number of events"
	labelVars[c("Mean", "statMean")] <- paste("Mean", getLabelVar(colorVar[countVar], labelVars = labelVars))

```

```{r countsVisualizationTemplate-createPlot}
	
	# format label for 'knitPrintMedicalMonitoring'
	...
	listPlots <- sapply(typePlot, function(typePlotI){
		...
		listPlots <- dlply(dataPlot, loopingVar, function(dataI){
			...
			# create plot
			colorLab <- toString(getLabelVar(colorVar[countVar], data = dataI, labelVars = labelVars))
			pathLab <- unname(getLabelVar(var = "USUBJID", labelVars = labelVars))
			plotCountMonitoring(
				data = dataI,
				vars = c(parentVar, countVar),
	...
	names(listPlots) <- paste(simpleCap(typePlot), "visualization")
