...
data <- dlply(dataCurrent, loopingVar, function(dataCurrentI){
	...
	switch(comparisonTableType,
		...
		`table-comparison-interactive` = {
			...
			argsExportDiff <- c(
				...
				if(!is.null(labelVars))
					list(colnames = setNames(names(labelVars), labelVars)),
				if(!is.null(patientProfilePath))
					list(escape = match("USUBJID", colnames(tableComparison))-1)
			)
		...
		# TODO: replace by 'newData-diff-interactive' output type
		`newData-diff` = {
		
			tableParams$tableVars <- c("Comparison type", tableParams$tableVars)		
			argsTable <- c(
				list(
			...
			tableComparisonDT <- do.call("tableMonitoring", argsTable)	
