...
data <- dlply(dataCurrent, loopingVar, function(dataCurrentI){
	...
	tableLab <- getLabelVar(var = tableVars, labelVars = labelVars, label = tableParams$tableLab)
	if(!is.null(patientProfilePath)){
	...
	# get columns ordered to have patientProfileLink just after USUBJID column
	getColsOrderedPatientProfiles <- function(data){
		cols <- colnames(data)
		idxColPPLink <- match("patientProfileLink", cols)
		idxColUSUBJID <- match("USUBJID", cols)
		idxColsSorted <- union(c(seq_len(idxColUSUBJID), idxColPPLink), seq_len(ncol(dataCurrentI)))
		return(cols[idxColsSorted])
	}
	
	# formatting for patient profiles
	getArgsExportDiffPatientProfiles <- function(data, argsExportDiff){
	...
	switch(comparisonTableType, 
		...
		`newData-diff-interactive` = {
			...
			if(!is.null(patientProfilePath)){
				...
				cols <- getColsOrderedPatientProfiles(data = dataCurrentI)
				dataCurrentI <- subset(dataCurrentI, select = cols)
				argsExportDiff <- getArgsExportDiffPatientProfiles(
					data = dataCurrentI, 
		...
		`table-comparison-interactive` = {
			...
			# formatting for patient profiles:
			cols <- getColsOrderedPatientProfiles(data = tableComparison)
			tableComparison <- tableComparison[, cols, drop = FALSE]
			argsExportDiff <- getArgsExportDiffPatientProfiles(
