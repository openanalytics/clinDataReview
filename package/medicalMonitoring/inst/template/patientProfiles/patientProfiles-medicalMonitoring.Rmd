...
```{r patientProfiles-runDocument-example, eval = FALSE, echo = FALSE}
	...
	outputFile <- file.path("../report", "patientProfiles", "subjectProfile.pdf")
...
```{r patientProfiles-loadPackages, message = FALSE}

	library(patientProfilesVis)

```

```{r patientProfiles-loadData, message = FALSE}
...
```{r patientProfiles-subsetData}	
	...
	if(!is.null(params$subsetDataset)){
		...
		if(params$subsetDataset %in% names(dataAll)){
			...
			if(length(subjectSubset) == 0)
				stop("No subjects are retained after filtering.")
...
```{r patientProfiles-createPatientProfiles-dm, eval = "dm" %in% patientProfileModules}
	...
	cat("Subject profiles created for demographics for", length(dmPlots), "subject(s).")
	
	
	rm(list = c("dataDM", "dmPlots"));tmp <- gc(verbose = FALSE) # free memory
```
...
```{r patientProfiles-formatData-MH, eval = any(c("ADMH", "MH") %in% names(dataAll))}
	...
	varMHAENDT <- intersect(c("AENDT", "MHENDTC"), colnames(dataMH))[1]
...
```{r patientProfiles-createPatientProfiles-mh, eval = "mh" %in% patientProfileModules}
	...
	cat("Subject profiles created for medical history for", length(mhPlots), "subject(s).")
	
	rm(list = c("dataMH", "mhPlots"));tmp <- gc(verbose = FALSE)# free memory
...
```{r patientProfiles-formatData-CM, eval = any(c("ADCM", "CM") %in% names(dataAll))}	
	...
	varCMCode <- intersect(c("ADECOD", "CMDECOD"), colnames(dataCM))[1]
	varCM <- if(length(varCMCode) > 0){
		...
		dataCM[, "CMTERM"] <- with(dataCM, ifelse(CMTERM == "", "No CM term provided.", CMTERM))
		"CMTERM"
	}
...
```{r patientProfiles-createPatientProfiles-cm, eval = "cm" %in% patientProfileModules}
	...
	cat("Subject profiles created for concomitant medications for", length(cmPlots), "subject(s).")
	
	rm(list = c("dataCM", "cmPlots"));tmp <- gc(verbose = FALSE) # free memory
...
```{r patientProfiles-createPatientProfiles-ex, eval = "ex" %in% patientProfileModules}
	...
	cat("Subject profiles created for exposure for", length(exPlots), "subject(s).")
	
	rm(list = c("dataEX", "exPlots"));tmp <- gc(verbose = FALSE) # free memory
```
...
```{r patientProfiles-formatData-AE, eval = any(c("ADAE", "AE") %in% names(dataAll))}	
	...
	colorVarAE <- intersect(c("ASEV", "AESEV", "AETOXGR"), colnames(dataAE))[1]
	colorPaletteAE <- colorPalette[c("green", "sand", "yellow", "orange", "signalRed", "grey")]
	if(colorVarAE %in% c("ASEV", "AESEV")){
		indLevels <- c("Mild", "Moderate", "Severe", "Life-threatening", "Fatal")
		dataAE[, colorVarAE] <- if("ASEVN" %in% colnames(dataAE)){
			reorder(dataAE[, colorVarAE], dataAE[, "ASEVN"])
		}else{
			xVarAE <- unname(simpleCap(tolower(dataAE[, colorVarAE])))
			levelsAE <- c(intersect(indLevels, unique(xVarAE)), setdiff(xVarAE, indLevels))
			factor(xVarAE, levels = levelsAE)
		}
		dataAE[, colorVarAE] <- convertAesVar(dataAE, var = colorVarAE)
	}else	if(colorVarAE %in% "AETOXGR"){
		indLevels <- as.character(1:5)
		dataAE[, colorVarAE] <- convertAesVar(dataAE, var = colorVarAE)
	}else	colorVarAE <- NULL
...
```{r patientProfiles-createPatientProfiles-ae, eval = "ae" %in% patientProfileModules}
	...
	cat("Subject profiles created for adverse events for", length(aePlots), "subject(s).")
	
	rm(list = c("dataAE", "aePlots"));tmp <- gc(verbose = FALSE) # free memory
...
```{r patientProfiles-createPatientProfiles-lb, eval = "lb" %in% patientProfileModules}
	...
	cat("Subject profiles created for laboratory parameters for", length(lbPlots), "subject(s).")
	
	rm(list = c("dataLB", "lbPlots"));tmp <- gc(verbose = FALSE) # free memory
...
```{r patientProfiles-createPatientProfiles-vs, eval = "vs" %in% patientProfileModules}
	...
	cat("Subject profiles created for vital signs for", length(vsPlots), "subject(s).")
	
	rm(list = c("dataVS", "vsPlots"));tmp <- gc(verbose = FALSE) # free memory
...
```{r patientProfiles-createPatientProfiles-eg, eval = "eg" %in% patientProfileModules}
	...
	cat("Subject profiles created for exposure for", length(egPlots), "subject(s).")
	
	rm(list = c("dataEG", "egPlots"));tmp <- gc(verbose = FALSE) # free memory
