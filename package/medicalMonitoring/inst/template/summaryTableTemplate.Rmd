...
```{r summaryTableTemplate-exportSummaryStatisticsTable, eval = (comparisonTableType == "none")}
	...
	summaryTableDT <- sapply(names(summaryTable), function(group){
		...
		sumTable <- summaryTable[[group]]
		# export to docx		
		tableParamsDocxI <-tableParamsDocx
		tableParamsDocxI$summaryTable <- sumTable
		
		if(is.null(tableParamsDocxI$file))	
			tableParamsDocxI$file <- "table.docx"
...
summaryTableDT <- sapply(names(summaryTableList$currentData$summaryTable), function(group){
	...
	sumTableCurrent <- summaryTableList$currentData$summaryTable[[group]]
	sumTableOld <- summaryTableList$previousData$summaryTable[[group]]
