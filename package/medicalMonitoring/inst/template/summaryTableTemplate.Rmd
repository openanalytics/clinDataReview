...
- comparisonTableType (optional): string with type of comparison table, see 'outputType' in ? glpgUtilityFct::compareTables
...
summaryTableList <- sapply(names(pathsData), function(dataBatch) {
      ...
      summaryTable <- dlply(data, loopingVar, function(dataI){
			...
			if("colVarDataLevels" %in% names(argsTable)){
				argsTable$colVarDataLevels <- eval(expr = parse(text = argsTable$colVarDataLevels))
			}
...
summaryTableDT <- sapply(names(summaryTable), function(group){
      ...
      tableParamsDTI$title <- htmltools::tags$caption(
          htmltools::a("Formatted .docx table", target="_blank", href = tableParamsDocxI$file),
          br(),
          ...
          if(!is.null(loopingVar) && loopingTotal)	
            tagList(
                ...
                paste0("Totals (percentages) are considered by ", 
                    toString(getLabelVar(loopingVar, labelVars = labelVars)), 
                    "."
                )
            )
      )          
...
summaryTableDT <- sapply(names(summaryTableList$currentData$summaryTable), function(group){
	  ...
	  if(!is.null(labelVars))
		  argsCompTable$colnames <- setNames(labelVars, labelVars)
