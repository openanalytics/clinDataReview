...
These dependencies are included in the `medicalMonitoring` package, and are listed
with the convenient function: `getJsDepMedicalMonitoring()`.
They should be specified as extra dependencies during the report creation
(`extra_dependencies`) as in the code below:
...
```{r runReport, eval = FALSE}
	...
	createPatientProfiles <- FALSE # doesn't create patient profiles at each re-run
	rmarkdown::render(
...
```{r createPatientProfilesReport, eval = createPatientProfiles}
...
A table of counts should be provided for the sunburst, treemap and barplot visualizations.
...
```{r treemap}
	...
	# Reformat summary statistics data.frame to include the label
	# of each group in the AEDECOD column for the total
	dataTreemap['parent'] = with(dataTreemap, 
		ifelse(AEDECOD == "Total", 'Adverse events', as.character(AESOC))
	)
	dataTreemap['child'] = with(dataTreemap, 
		ifelse(AEDECOD == "Total", as.character(AESOC), as.character(AEDECOD))
	)	
	treemapMonitoring(
		data = dataTreemap,
