---
title: "Reporting with the `clinDataReview` package"
...
vignette: >
  %\VignetteIndexEntry{Reporting with the clinDataReview package}
...
Utility functions are provided to run a standard clinical data report.
...
The clinical data report output consists of a final output and a set of
...
available at: `? `clinDataReview-templates` `**.
...
```
The `render_clinDataReviewReport` creates several html files, one per
section/subsection.

To open the full report, the user should open the _introduction.html_ file.

However, if several files are available, it might be boring to look for the
_introduction.html_ among many files. In such a case, it can be created a
landing page, as described in the next section. 
