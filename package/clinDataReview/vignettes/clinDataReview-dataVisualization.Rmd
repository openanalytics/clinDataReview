...
Please note that the patient profiles are only created if Pandoc
is available.

```{r createPatientProfiles, results = "hide", eval = rmarkdown::pandoc_available()}
...
tableAE <- getSummaryStatisticsTable(
	...
	# should be specified (rowVarTotalInclude = 'AEDECOD')
	# The total across the other variable(s) is optional
...
dataTreemap$`%` <- as.numeric(dataTreemap$`%`)
