---
title: "Data visualization of the `clinDataReview` package"
...
vignette: >
  %\VignetteIndexEntry{Data visualization of the clinDataReview package}
...
This vignette focuses on the visualization functionalities of the
`clinDataReview` package.

We will use example data sets from the `clinUtils` package.

If you have doubts on the data format, first check out the vignette on data
preprocessing avaialable
[here](../doc/clinDataReview-dataPreprocessingVignette.html)).
...
The documentation on how to set up a config file is available in the vignette
about reporting.

You can directly skip to reporting vignette, which is available
[here](../doc/clinDataReview-reportingVignette.html) or run in your
console the command below.

```{r reportingVignette, eval = FALSE}

vignette("clinDataReview-reportingVignette", "clinDataReview")
...
scatterplotMonitoring(
    ...
    xVar = "ADY",
    yVar = "LBSTRESN",
    aesPointVar = list(color = "ARM", fill = "ARM"),
...
scatterplotMonitoring(
    ...
    aesPointVar = list(color = "VISIT", fill = "VISIT", shape = "ARM"),
...
The **`sunburstMonitoring`** function visualizes the counts of hierarchical data
in nested circles.
